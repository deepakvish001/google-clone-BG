...
Define the screen reader flow standard for SearchCanvas so the lightweight search interface remains accessible, privacy-conscious, and predictable across browsers.
...
- The screen reader flow requirement is explicit and observable.
...
Keep implementation work focused on one screen reader flow outcome and avoid copying third-party branding or behavior without review.