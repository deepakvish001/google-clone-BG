...
Define the provider selection standard for SearchCanvas so the lightweight search interface remains accessible, privacy-conscious, and predictable across browsers.
...
- The provider selection requirement is explicit and observable.
...
Keep implementation work focused on one provider selection outcome and avoid copying third-party branding or behavior without review.