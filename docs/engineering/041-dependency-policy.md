...
Define the dependency policy standard for SearchCanvas so the lightweight search interface remains accessible, privacy-conscious, and predictable across browsers.
...
- The dependency policy requirement is explicit and observable.
...
Keep implementation work focused on one dependency policy outcome and avoid copying third-party branding or behavior without review.